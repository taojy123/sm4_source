...
namespace ConsoleApp1
{
    class SM4
    {
        private static uint[] SM4_CK = { 0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1,
        ...
        private static byte[] SM4_Sbox = { (byte) 0xd6, (byte) 0x90, (byte) 0xe9, (byte) 0xfe, (byte) 0xcc,
        ...
        private static uint[] SM4_FK = { 0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC };

        private static int SM4_Rotl32(int buf, int n)
        {
            return (int)(((uint)buf << n) | ((uint)buf >> (32 - n)));
        }
        ...
        public static void SM4_KeySchedule(byte[] MK, int[] rk)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                K[i] = (int)(SM4_FK[i] ^ ((MK[4 * i] << 24) | (MK[4 * i + 1] << 16) | (MK[4 * i + 2] << 8) | (MK[4 * i + 3])));
            }
            for (i = 0; i < 32; i++)
            {
                tmp = (int)(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ SM4_CK[i]);
                buf = (int)(
